...
curl -fLs "https://github.com/docker/machine/releases/download/$version/docker-machine-Linux-x86_64" > /tmp/docker-machine
...
parallel -d ' ' -j 200% " \
curl -fLs https://raw.githubusercontent.com/docker/machine/$version/contrib/completion/bash/{} > /tmp/{} && \
...
curl -fLs "https://storage.googleapis.com/kubernetes-release/release/$version/bin/linux/amd64/kubectl" > /tmp/kubectl
install -o root -g root -m 755 /tmp/kubectl /usr/local/bin/kubectl
rm /tmp/kubectl

# install Skaffold
curl -fLs skaffold 'https://storage.googleapis.com/skaffold/releases/latest/skaffold-linux-amd64' > /tmp/skaffold
